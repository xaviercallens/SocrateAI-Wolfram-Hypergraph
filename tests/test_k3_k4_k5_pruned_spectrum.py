...
def test_k3_k4_k5_pruned_spectrum_trial():
    ...
    res = agent.execute_k3_k4_k5_mass_spectrum_trial_pruned(
        iterations=10, pruning_mode="aggressive")
    ...
    print(
        f"Pruning Efficiency: {res['hardware_accelerator']['pruning_efficiency_ratio']}")
    print(
        f"K3 Integrity: {spec['K3_triangle']['final_integrity']} (Evaporated)")
    print(
        f"K4 Integrity: {spec['K4_tetrahedron']['final_integrity']} (Stable Soliton)")
    print(
        f"K5 Integrity: {spec['K5_pentagram']['final_integrity']} (Deep Gravity Well R={spec['K5_pentagram']['curvature_ratio_R']})")
