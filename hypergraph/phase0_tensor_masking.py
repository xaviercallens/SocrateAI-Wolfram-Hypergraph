...
def generate_topological_mask(M_t: torch.Tensor, k4_indices: list = [0, 1, 2, 3]) -> torch.Tensor:
    """
    ...
    # Preserve original 1-hop background neighborhood structure (linear background)
...
def run_phase0_simulation(max_steps: int = 15, device: str = None) -> dict:
    """
    ...
    print(f"=== Running Phase 0 PyTorch Tensor Masking MVP on Device: {device} ===")
    ...
    for step in range(1, max_steps + 1):
        ...
        # 2. Execute PyTorch Hadamard rewrite update: M_{t+1} = (M_t^2 + M_t) (o) T
        ...
        print(f"Step {step:2d} | Edges: {non_zero_edges:4d} | Unmasked Sum: {unmasked_sum:10.2f} | Masked Sum: {masked_sum:8.2f} | Hash: {current_hash[:8]} | Pruned: {is_pruned}")
