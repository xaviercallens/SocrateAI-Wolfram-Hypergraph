...
class WolframRateLimiter:
    ...
    def _refill_tokens(self):
        ...
        self.tokens = min(float(self.burst_capacity), self.tokens + elapsed * self.refill_rate)
    ...
    def execute_with_rate_limit(self, func, *args, **kwargs):
        ...
        for attempt in range(1, self.retry_attempts + 1):
            try:
                ...
                sleep_duration = (self.backoff_factor ** attempt) + random.uniform(0.1, 1.0)
                if is_rate_limit:
                    sleep_duration *= 2.0  # Double wait if explicitly rate-limited
                    
                print(f"⚠️ Wolfram API call failed (Attempt {attempt}/{self.retry_attempts}): {err}. Retrying in {sleep_duration:.2f}s...")
