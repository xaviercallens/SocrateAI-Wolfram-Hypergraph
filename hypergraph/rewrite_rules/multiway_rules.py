...
class MultiWayRule:
    ...
    def generate_multiway_step(self, edges: List[Tuple[int, ...]], max_branches: int = 4) -> Dict[str, Any]:
        """
        ...
        if edges:
            ...
            branches.append({"branch_id": 1, "rule_applied": "local_division", "edges": b1_edges})
        ...
        if len(edges) >= 2:
            ...
            branches.append({"branch_id": 2, "rule_applied": "tangle_merge", "edges": b2_edges})

        # Branch 3: Oligon tangle creation
        if len(edges) >= 3:
            b3_edges = list(edges[3:])
            z4 = node_max + 4
            b3_edges.extend([(edges[0][0], edges[1][0], z4), (edges[1][0], edges[2][0], z4), (edges[2][0], edges[0][0], z4)])
            branches.append({"branch_id": 3, "rule_applied": "oligon_defect_spawn", "edges": b3_edges})
