...
def run_local_dry_run(
    ...
    print(f"💰 Hourly Burn Rate Estimate: ${cost_monitor.total_hourly_burn_rate:.4f} / hr")
    ...
    for step in range(1, max_steps + 1):
        ...
        if step % 5 == 0 or step == max_steps:
            snapshot_file = cost_monitor.storage_dir / f"snapshot_step_{step}.pt"
            torch.save(M_next.cpu(), snapshot_file)
            
        print(f"Step {step:2d}/{max_steps} | Edges: {non_zero_edges:3d} | Masked Sum: {masked_sum:8.2f} | Unmasked Sum: {unmasked_sum:10.2f} | VRAM: {vram_mb:6.1f} MB | Hash: {curr_hash[:8]} | Pruned: {is_pruned}")
    ...
    summary_report = {
        ...
        "disk_log_mount": str(cost_monitor.storage_dir),
        "wolfram_queries_issued": wolfram_limiter.total_queries_issued,
        "wolfram_total_wait_sec": round(wolfram_limiter.total_throttle_wait_sec, 3),
    ...
    print(f"💰 Total Estimated Dry Run Cost: ${cost_summary['total_cost_usd']:.6f} USD")
    print(f"⚡ Unique Isomorphic States: {len(seen_hashes)} | Wolfram Queries: {wolfram_limiter.total_queries_issued}")
    print("=================================================================")
