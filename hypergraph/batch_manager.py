...
BATCH_STORAGE_DIR = Path("/mnt/disks/disk-socrateai-local-1/hypergraph_logs/batch_runs")
...
class BatchManager:
    ...
    def run_batch(self) -> dict:
        ...
        print(f"Target Duration: {self.duration_seconds / 60.0:.1f} minutes ({self.duration_seconds:.0f} seconds)")
        ...
        M_t = create_k4_oligon_seed(vacuum_size=self.vacuum_size).to(self.device)
        ...
        while time.time() < end_time:
            ...
            if step % self.snapshot_interval_steps == 0 or remaining_time <= 0:
                checkpoint_file = self.storage_dir / f"checkpoint_step_{step}.pt"
            ...
            status_payload = {
                ...
                "progress_pct": round((elapsed / self.duration_seconds) * 100.0, 1),
        ...
        print("=================================================================")
        print(f"✅ Batch Manager Run Finished ({step} steps in {total_elapsed:.1f}s)")
        print(f"Summary Report: {self.storage_dir / 'batch_final_summary.json'}")
        print("=================================================================")
...
if __name__ == "__main__":
    ...
    dur = float(sys.argv[1]) if len(sys.argv) > 1 else 60.0  # 60 sec test default, or 3600 for full hour
