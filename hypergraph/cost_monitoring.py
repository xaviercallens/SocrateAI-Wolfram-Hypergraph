...
REGION_SPOT_PRICING = {
    "us-central1": {"t4_gpu": 0.110, "vcpu_hr": 0.008, "ram_gb_hr": 0.001, "disk_gb_mo": 0.040},
    "us-east1":    {"t4_gpu": 0.110, "vcpu_hr": 0.008, "ram_gb_hr": 0.001, "disk_gb_mo": 0.040},
    "us-west1":    {"t4_gpu": 0.110, "vcpu_hr": 0.008, "ram_gb_hr": 0.001, "disk_gb_mo": 0.040},
    "europe-west4":{"t4_gpu": 0.125, "vcpu_hr": 0.009, "ram_gb_hr": 0.0012, "disk_gb_mo": 0.044},
}

DEFAULT_STORAGE_PATH = Path("/mnt/disks/disk-socrateai-local-1/hypergraph_logs")
...
class GlobalCostMonitor:
    def __init__(
        ...
        self.disk_hourly_cost = (self.disk_gb * self.pricing["disk_gb_mo"]) / 730.0  # 730 hrs/month
        
        self.total_hourly_burn_rate = (
            self.gpu_hourly_cost + self.cpu_hourly_cost + self.ram_hourly_cost + self.disk_hourly_cost
        )
    ...
    def calculate_current_cost(self) -> dict:
        ...
        compute_cost = elapsed_hrs * (self.gpu_hourly_cost + self.cpu_hourly_cost + self.ram_hourly_cost)
        ...
        remaining_compute_hrs = remaining_budget / max(0.001, self.total_hourly_burn_rate)
        
        return {
            "region": self.region,
            "elapsed_seconds": round(time.time() - self.start_time, 2),
            "elapsed_hours": round(elapsed_hrs, 4),
            "hourly_burn_rate_usd": round(self.total_hourly_burn_rate, 4),
            "total_cost_usd": round(total_cost, 6),
            "remaining_budget_usd": round(remaining_budget, 4),
            "max_remaining_compute_hours": round(remaining_compute_hrs, 2),
            "budget_used_pct": round((total_cost / self.max_budget_usd) * 100.0, 2),
        }

    def log_telemetry(self, step_info: dict = None) -> Path:
    ...
    def enforce_budget_guardrails(self) -> bool:
        ...
        if cost_info["total_cost_usd"] >= self.max_budget_usd:
            print(f"⚠️ BUDGET EXCEEDED: Total spend ${cost_info['total_cost_usd']:.2f} >= Cap ${self.max_budget_usd:.2f}. Triggering graceful shutdown!")
            return False
        return True


if __name__ == "__main__":
    monitor = GlobalCostMonitor(region="us-central1", disk_gb=500.0, max_budget_usd=100.0)
